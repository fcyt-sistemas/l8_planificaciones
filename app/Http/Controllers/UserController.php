...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->user()->hasRole('admin')) {
            $usuarios = User::name($request->get('name'))->email($request->get('email'))->role_user($request->get('role_user'))->orderBy('users.name', 'DESC')
            ->paginate(5);
            $roles = Role::all();
    ...
    public function update($id, Request $request)
    {
        ...
        $rol = Role::where('name', $request->roles)->first();
